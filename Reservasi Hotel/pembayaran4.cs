...
{
    public partial class pembayaran4 : Form
    {
        ...
        public pembayaran4(int nomor_kamar, List<string> id_tamu, List<string> nama_tamu, int sisaBayar)
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for(int i = 0; i < id_tamu.Count; i++)
                {
                    cari_id_transaksi_dan_reservasi(nomor_kamar, id_tamu[i]);
                    if (comboBox1.SelectedIndex != i)
                    {
                        SQL = "UPDATE transaksi SET tgl_keluar='" + tgl + "', jam_keluar='" + jam + "', jumlah_bayar='" + 0 + "' WHERE id_transaksi=" + id_trx + ";";
                        conn.Open();
                        cmd = new MySqlCommand(SQL, conn);
                        reader = cmd.ExecuteReader();
                        while (reader.Read())
